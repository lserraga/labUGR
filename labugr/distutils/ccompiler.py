...
from labugr.distutils.exec_command import exec_command
from labugr.distutils.misc_util import cyg2win32, is_sequence, mingw32, \
...
def CCompiler_spawn(self, cmd, display=None):
    ...
    display : str or sequence of str, optional
        The text to add to the log file kept by `labugr.distutils`.
...
def CCompiler_compile(self, sources, output_dir=None, macros=None,
    ...
    if sys.version_info[0] < 3:
        ...
        from labugr.distutils.fcompiler import (FCompiler, is_f_file,
                                               has_f90_header)
...
def new_compiler (plat=None,
                  ...
                  dry_run=0,
                  force=0):
    # Try first C compilers from labugr.distutils.
    if plat is None:
        plat = os.name
    try:
        if compiler is None:
    ...
    module_name = "labugr.distutils." + module_name
    try:
        __import__ (module_name)
    except ImportError:
        msg = str(get_exception())
        log.info('%s in labugr.distutils; trying from distutils',
                 str(msg))
