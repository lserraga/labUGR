...
from labugr.audio.wavfile import read, write, WavFileWarning
...
def test_read_1():
    for mmap in [False, True]:
        rate, data = read(datafile('test-44100Hz-le-1ch-4bytes.wav'),
                                  mmap=mmap)
...
def test_read_2():
    for mmap in [False, True]:
        rate, data = read(datafile('test-8000Hz-le-2ch-1byteu.wav'),
                                  mmap=mmap)
...
def test_read_3():
    for mmap in [False, True]:
        rate, data = read(datafile('test-44100Hz-2ch-32bit-float-le.wav'),
                                  mmap=mmap)
        assert_equal(rate, 44100)
...
def test_read_4():
    for mmap in [False, True]:
        with suppress_warnings() as sup:
            sup.filter(WavFileWarning,
                       "Chunk .non-data. not understood, skipping it")
            rate, data = read(datafile('test-48000Hz-2ch-64bit-float-le-wavex.wav'),
                                      mmap=mmap)
...
def test_read_5():
    for mmap in [False, True]:
        rate, data = read(datafile('test-44100Hz-2ch-32bit-float-be.wav'),
                                  mmap=mmap)
        assert_equal(rate, 44100)
        assert_(np.issubdtype(data.dtype, np.float32))
...
def test_read_fail():
    for mmap in [False, True]:
        ...
        assert_raises(ValueError, read, fp, mmap=mmap)
        fp.close()
...
def _check_roundtrip(realfile, rate, dtype, channels):
    ...
    try:
        ...
        write(tmpfile, rate, data)

        for mmap in [False, True]:
            rate2, data2 = read(tmpfile, mmap=mmap)
